...
export default function OrderOnline() {
  ...
  return (
    <section className="container" aria-label="Order Online Section">
      ...
      {/* Filters for search and type */}
      <Filters
        query={query}
        setQuery={(val) => dispatch(setQuery(val))}
        veg={veg}
        setVeg={(val) => dispatch(setVeg(val))}
      />
            ...
            <div className="controls-column">
              ...
              {qty > 0 && (
                <div style={{ marginTop: 8 }}>
                  <QuantityStepper
                    value={qty}
                    onChange={(v) => dispatch(updateItem({ id: item.id, qty: v }))}
          ...
          <ul className="plain" aria-label="Order summary list">
            {summary.items.map((item) => (
              <li
                ...
                aria-label={`${item.name}, Quantity: ${item.qty}, Price: ₹${item.price * item.qty}`}
              >
                <span>
                  {item.name} × {item.qty}
                </span>
